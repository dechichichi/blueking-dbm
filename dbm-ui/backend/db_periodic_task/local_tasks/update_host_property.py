...
def batch_generator_large(queryset, batch_size=2):
    iterator = queryset.iterator()
    while batch := list(islice(iterator, batch_size)):
        yield batch


@register_periodic_task(run_every=crontab(minute=30))
def update_all_host_property():
    try:
        machines = Machine.objects.all()
        machine_count = machines.count()
        task_count = machine_count // 2 if not machine_count % 2 else machine_count // 2 + 1

        for index, batch in enumerate(batch_generator_large(machines, 2)):
            countdown = calculate_countdown(count=task_count, index=index, duration=12 * TimeUnit.MINUTE)
...
def update_machine_field(bk_host_ids):
    logger.info(f"111111111111111111111, begin to update machine field:{bk_host_ids}")
    # 初始化请求参数
    params = {"bk_fields": DEFAULT_BK_FIELDS, "page": {"start": 0, "limit": 10}}
